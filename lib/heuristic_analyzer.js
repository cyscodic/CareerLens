...
export function analyzeHeuristic(resumeText, jobDescription = '') {
  ...
  // Role recommendations (expanded — always 5+)
  ...
  const hasML = techInResume.some(t => ['pytorch','tensorflow','sklearn','llm','rag','embeddings','transformers'].includes(t))
  const hasMobile = techInResume.some(t => ['swift','kotlin','flutter','react-native'].includes(t))
  ...
  if (hasFE) role_recommendations.push({ role: 'UI/UX Engineer', fit_score: 70, why: 'Frontend chops + design sensibility implied.' })
  if (hasBE) role_recommendations.push({ role: 'Backend Engineer', fit_score: 80, why: 'Server-side stack present.' })
  if (hasBE && hasDevops) role_recommendations.push({ role: 'Platform / Infrastructure Engineer', fit_score: 82, why: 'Backend + cloud/infra tooling combo.' })
  if (hasData) role_recommendations.push({ role: 'Data Engineer', fit_score: 78, why: 'Data tooling present in resume.' })
  if (hasML) role_recommendations.push({ role: 'ML Engineer', fit_score: 80, why: 'ML libraries detected.' })
  if (hasDevops) role_recommendations.push({ role: 'DevOps / SRE', fit_score: 75, why: 'Cloud & infra tooling detected.' })
  if (hasMobile) role_recommendations.push({ role: 'Mobile Engineer', fit_score: 78, why: 'Mobile dev stack detected.' })
  // Generalist defaults always include
  role_recommendations.push({ role: 'Software Engineer (Generalist)', fit_score: 65, why: 'Broad fit across software roles.' })
  if (expYears >= 5) role_recommendations.push({ role: 'Senior Software Engineer', fit_score: 78, why: `${expYears}+ years of experience supports a senior level.` })
  if (expYears >= 7) role_recommendations.push({ role: 'Tech Lead', fit_score: 72, why: `Experience level (${expYears}y) opens lead roles.` })
